...
describe('the inline-style gate rejects a spelling of the name', () => {
...
describe('the inline-style gate rejects an opaque write', () => {
...
describe('the inline-style gate rejects the attribute in markup', () => {
  ...
  it('the attribute written in JSX, in the dialects that carry it', () => {
    expect(styleOffences(jsxBadge('style'), 'badge.tsx')).not.toEqual([])
    expect(styleOffences(jsxBadge('className'), 'badge.tsx')).toEqual([])
  })
...
describe('the inline-style gate reads every file it is given', () => {
