...
const WHOLE = numbered([2, 4, 6, 8, 12, 16, 20, 24, 32, 48])

/** A token sheet holding the declarations a case writes, one per line. */
function tokens(...declarations: readonly string[]): string {
  return `:root {\n${declarations.map((one) => `  ${one};`).join('\n')}\n}\n`
}

/** The rungs a case declares, with one of them holding another value. */
function withValue(declarations: readonly string[], property: string, value: string): string[] {
 ...
 */
const REFUSED_SCALE: readonly ScaleCase[] = [
  {
  ...
  },
  {
    ...
    why: [
      '--dsh-tracking-brand is declared twice under :root; the second replaces the first, so neither is the one decision',
    ],
  },
  {
...
const REFUSED_READER: readonly ReaderCase[] = [
 ...
 */
function misreported<Case extends { readonly why: Reasons }>(
  cases: readonly Case[],
...
const reason = (offence: Offence): string => offence.why

describe('the scale gate rejects', () => {
  it('every declaration the scale does not hold, and every decision stated beside it', () => {
    expect(misreported(REFUSED_SCALE, (one) => scaleOffences(TOKEN_SHEET, tokens(...one.writes)).map(reason))).toEqual(
      [],
    )
    expect(misreported(REFUSED_READER, (one) => referenceOffences(SHEET, one.writes).map(reason))).toEqual([])
  })
...
describe('the gate the chain runs', () => {
  it('reads the token sheet as the scale and every other sheet as a reader of it', () => {
    ...
    expect(scanScale(SHEET, reader).map(reason)).toEqual([why])
    expect(scanScale(TOKEN_SHEET, reader).map(reason)).not.toContain(why)
