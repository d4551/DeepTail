...
it('dismisses the menu when a pointer lands outside it, without taking focus back', async () => {
  ...
  expect(
    await page.evaluate(() => {
      const connection = 'deeptailConnection'
      const active = document.activeElement
      return active instanceof HTMLElement ? active.dataset[connection] : undefined
    }),
  ).not.toBe('trigger')
  await page.close()
})
...
it('holds the pane behind the open drawer out of reach', async () => {
  ...
  await page.waitForFunction(() => {
    ...
    return (active instanceof HTMLElement ? active.dataset[action] : undefined) === 'drawer'
  })
