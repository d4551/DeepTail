...
it('dismisses the menu when a pointer lands outside it, without taking focus back', async () => {
  ...
  expect(
    await page.evaluate(() => {
      ...
      return active instanceof HTMLElement ? active.dataset['deeptailConnection'] : undefined
    }),
  ).not.toBe('trigger')
  await page.close()
})
...
it('holds the pane behind the open drawer out of reach', async () => {
  ...
  await page.waitForFunction(() => {
    const active = document.activeElement
    return (active instanceof HTMLElement ? active.dataset['deeptailAction'] : undefined) === 'drawer'
  })
